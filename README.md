...
3. Start the game. `BepInEx\LogOutput.log` should contain
   ...
   ```
   [Info   :Client Side Damage] Client Side Damage 1.4.4 loaded (protocol 10)
   ```
...
3. Play co-op. When a modded client joins a modded host, the logs show
   ...
   (`v1.4.4 loaded on your side, waiting for the host...`, then `host enabled: ...`, or
   ...
   creates a multiplayer lobby (`CSD : v1.4.4 host ON: guard/dodge, bullets, melee, area, fresh-pos`)
